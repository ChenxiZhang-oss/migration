...
# 定义日期范围 (10号到20号)
dates = [f'2020010{i}' for i in range(10)] + [f'202001{i}' for i in range(10, 21)]

# 收集所有日期的数据
all_data = []

for date in dates:
    # 文件名: city_迁出_省百分比_YYYYMMDD.json
    json_file = data_dir / f'city_迁出_省百分比_{date}.json'
...
# 保存结果CSV
output_file = Path('./data/武汉市迁出省级汇总_20200110-20200120.csv')
result_df.to_csv(output_file, index=False, encoding='utf-8-sig')

print(f'\n✓ 数据已保存到: {output_file}')
print(f'总共统计: {len(result_df)} 个省份')
print(f'统计日期: {len(dates)} 天 (20200110 - 20200120)')
print(f'\n数据预览:')
